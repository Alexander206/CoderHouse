...
function obtener(query = {}) {
  ...
  return filter(usuarios, (usuario) => {
    ...
    if (
      (query.correo && query.correo === usuario.correo) ||
      (query.telefono && query.telefono === usuario.telefono)
    ) {
      status = true;
    }
    return status;
...
function actualizarPorId(idUsuario, dataRequest) {
  ...
  const fieldTarget = ["nombre", "apellido", "edad", "telefono", "correo", "avatar", "estado"];
  const data = {
